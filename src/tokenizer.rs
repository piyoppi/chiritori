...
pub fn tokenize<'a, 'b, 'c>(source: &'a str, delimiter_start: &'b str, delimiter_end: &'c str) -> Vec<Token<'a, 'b, 'c>> {
    let (mut tokens, state, byte_start_pos, start_pos, current) = source.char_indices()
        .fold((vec![], State::Text, 0, 0, 0), |(mut tokens, state, mut byte_start_pos, mut start_pos, current): (Vec<Token<'a, 'b, 'c>>, State, usize, usize, usize), (byte_pos, c)| {
